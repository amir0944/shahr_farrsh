...
border: 1px solid;
	...
	<form action="#" style="margin: 40px 220px 40px 220px;margin: 40px 220px 40px 220px;
background-image: linear-gradient(red 0%,rgb(39, 39, 39)50%);
box-shadow: 2px 2px 50px gray;"  method="post">
		<label>ورود کاربر:</label><br>
...
if(isset($_POST['submit'])){
	...
	$pass=md5($_POST['password']);
	 $check=mysqli_query($db,"select * from users where username='$username' and password='$pass'");
	     if (mysqli_num_rows($check)>0){
        $_SESSION['user']=$username;
        header("Location: index.php");
    }
    else{
        echo "<script>alert('ثبت نام نکرده اید یا اطلاعات وارد شده اشتباه است.')</script>";
